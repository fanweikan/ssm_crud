...
public class EmployeeController {
	...
	/**
	 * 导入jackson包 把对象转换成JSON字符串 第二稿 使用 Bootstrap 之类的 CSS 框架，自动化实现 “响应式” 前端，支持移动设备
	 * 
	 * @param pn
	 * @return
	 */
	...
	/*
	 * /** 展示list.jsp页面 查询员工数据（分页查询） 第一稿 用静态方法刷新的页面，支持pc端浏览器
	 * 
	 * //@RequestMapping("/emps") public String getEmps(@RequestParam(value = "pn",
	 * defaultValue = "1") Integer pn ,Model model) { // 引入pagehelper分页插件 //
	 * 查询前调用，传入页码和记录数 PageHelper.startPage(pn, 5); // startPage紧跟着的这个查询就是一个分页查询
	 * List<Employee> emps = employeeService.getAll(); //
	 * PageInfo包装查询结果，封装了详细的分页信息和详细数据 // 连续显示5页 PageInfo pageInfo = new
	 * PageInfo(emps, 5); model.addAttribute("pageInfo", pageInfo); return "list"; }
	 */
	/**
	 * 保存员工信息 1.支持jsr303校验 2.导入hibrenate——validator包
	 */
